...
const statusLabel: Record<string, string> = {
  indexed: '已索引', processing: '处理中', pending: '等待中', failed: '失败',
};
  ...
  return (
    <div className="space-y-2.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-center gap-3.5 p-4 rounded-xl"
          style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)' }}>
          <div className="skeleton" style={{ width: 18, height: 18, borderRadius: 3, flexShrink: 0 }} />
          ...
          <div className="skeleton" style={{ width: 48, height: 22, borderRadius: 999, flexShrink: 0 }} />
        </div>
      ))}
    </div>
  );
...
export default function DocList({ refreshKey, kbId, searchProp }: { refreshKey: number; kbId?: string; searchProp?: string }) {
  ...
  const load = () => {
    setLoading(true);
    setError(null);
    api.get<any>(`/documents${kbId ? `?kb_id=${kbId}` : ''}`)
  ...
  const handleBatchDelete = async () => {
    ...
    finally { setBatchBusy(false); setBatchDeleteOpen(false); }
  };
  ...
  const handleCloseView = () => {
    setClosing(true);
    setTimeout(() => {
      setViewVisible(false); setClosing(false); setViewDoc(null);
  ...
  if (error) return (
    <div className="state-enter text-center py-12 px-8 rounded-xl"
      style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)', boxShadow: 'var(--shadow-sm)' }}>
      <div style={{ background: 'var(--c-error-subtle)', color: 'var(--c-error)' }}
      ...
      <p className="text-sm mb-5" style={{ color: 'var(--c-text-secondary)', maxWidth: 360, margin: '0 auto', lineHeight: 1.6 }}>
  ...
  if (docItems.length === 0) return (
    <div className="state-enter text-center py-16 px-8 rounded-xl"
      style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)', boxShadow: 'var(--shadow-sm)' }}>
      <div style={{ background: 'var(--c-primary-subtle)', color: 'var(--c-primary)' }}
        className="w-20 h-20 mx-auto mb-5 rounded-full flex items-center justify-center">
    ...
    <>
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 mb-3 px-1.5">
          ...
          <span className="text-xs" style={{ color: 'var(--c-text-secondary)' }}>已选 {selected.size} 项</span>
          <button onClick={() => setBatchDeleteOpen(true)} disabled={batchBusy}
            className="px-3 py-1 text-xs rounded border cursor-pointer disabled:opacity-50 transition-all"
            style={{
              color: 'var(--c-error)', background: 'var(--c-error-subtle)',
              borderColor: '#fecaca',
            }}>
            批量删除
          </button>
          <button onClick={handleBatchReindex} disabled={batchBusy}
            className="px-3 py-1 text-xs rounded border cursor-pointer disabled:opacity-50 transition-all"
            style={{
              color: 'var(--c-primary)', background: 'var(--c-primary-subtle)',
              borderColor: '#bfdbfe',
            }}>
            重新索引
          </button>
      ...
      {filtered.length === 0 && searchProp ? (
        <div className="text-center py-12 state-enter">
          ...
          <p className="text-sm" style={{ color: 'var(--c-text-secondary)' }}>尝试使用其他关键词搜索</p>
        </div>
      ) : (
        <div className="space-y-2.5 state-enter">
          {filtered.map((doc) => (
            <div key={doc.id}
              ...
              <span className={`shrink-0 text-xs font-medium px-2.5 py-1 rounded-full inline-flex items-center gap-1.5`}
                style={
                  doc.status === 'indexed'
                ...
                <span style={{
                  ...
                  background: 'currentColor', opacity: 0.75, display: 'inline-block',
                }} />
                {statusLabel[doc.status] || doc.status}
              </span>
              <button onClick={e => { e.stopPropagation(); setConfirmDeleteId(doc.id); }}
                ...
                onMouseLeave={e => { e.currentTarget.style.opacity = '0.7'; e.currentTarget.style.background = 'none'; e.currentTarget.style.color = 'var(--c-text-tertiary)'; }}
                title="删除">
      ...
      {/* View modal */}
      {viewDoc && viewVisible && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center ${closing ? 'animate-fade-out' : 'animate-fade-in'}`}
          style={{ background: 'rgba(0,0,0,.5)' }} onClick={handleCloseView}>
          <div className={`bg-white rounded-xl w-full max-w-2xl max-h-[80vh] flex flex-col mx-4 ${closing ? 'animate-scale-out' : 'animate-scale-in'}`}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b shrink-0">
              <div className="flex items-center gap-2">
                <h2 className="font-semibold">{viewDoc.title}</h2>
                <span className={`text-xs px-2 py-0.5 rounded-full inline-flex items-center gap-1 ${
                  viewDoc.status === 'indexed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                }`}>
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'currentColor', opacity: 0.75, display: 'inline-block' }} />
                  {statusLabel[viewDoc.status] || viewDoc.status}
                </span>
              </div>
              <button onClick={handleCloseView} className="p-1 rounded border-none cursor-pointer"
                style={{ color: 'var(--c-text-tertiary)', background: 'none' }}>
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-4 overflow-y-auto flex-1">
              {loadingContent ? (
                ...
                <pre className="text-sm whitespace-pre-wrap font-sans leading-relaxed">{viewDoc.content}</pre>
              )}
            </div>
          </div>
        </div>
      ...
      <ConfirmDialog open={!!confirmDeleteId} title="删除文档"
      ...
      <ConfirmDialog open={batchDeleteOpen}
        ...
        confirmText="全部删除" danger
        onConfirm={handleBatchDelete} onCancel={() => setBatchDeleteOpen(false)} />
    </>
  );
}
