...
export default function UploadDialog({ open, onClose, onUploaded, kbId }: Props) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setResult({ ok: true, msg: `${res.title} — 已加入索引队列` });
      setFile(null);
      if (inputRef.current) inputRef.current.value = '';
      onUploaded?.();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div
        className={`bg-white rounded-2xl p-6 w-full max-w-md mx-4 ${cardClass}`}
      ...
      >
        ...
        <label
          className="flex flex-col items-center justify-center gap-3 px-4 py-8 rounded-xl border-2 border-dashed cursor-pointer transition-all mb-4"
          style={{
            borderColor: file ? 'var(--c-primary)' : 'var(--c-border)',
            background: file ? 'var(--c-primary-subtle)' : 'var(--c-bg)',
          }}
        >
          {file ? (
        ...
        {uploading && (
          <div className="mb-4">
            ...
            <div className="w-full rounded-full h-2" style={{ background: 'var(--c-skeleton)' }}>
              <div
        ...
        {result && (
          <div
            className={`mb-4 px-4 py-2.5 rounded-xl text-sm flex items-center gap-2 ${result.ok ? 'bg-green-50 text-green-700 border border-green-200/50' : 'bg-red-50 text-red-600 border border-red-200/50'}`}
          >
            <span
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${result.ok ? 'bg-green-500' : 'bg-red-500'}`}
            />
            {result.msg}
          </div>
        )}
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={handleUpload} disabled={!file || uploading} className="btn-primary px-5">
