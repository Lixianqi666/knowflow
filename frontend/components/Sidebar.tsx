...
import {
  ChevronLeft, ChevronRight, Menu, X, Plus, type LucideIcon,
  MessageSquare, FileText, Bot, Settings, LogOut, Download, Trash2,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  const startRename = (id: string, title: string) => { setRenaming(id); setRenameValue(title || ''); };

  const submitRename = async (id: string) => {
    const val = renameValue.trim();
    if (val) {
      try { await api.patch(`/chat/conversations/${id}`, { title: val }); } catch {}
      useStore.setState((s) => ({
  ...
  const sidebarContent = sidebarCollapsed ? (
    <div className="flex flex-col items-center gap-3 py-3 h-full" style={{ color: 'var(--c-text-secondary)' }}>
      <button onClick={toggleSidebar} className="p-2 rounded-lg hover:bg-gray-100 transition-colors border-none cursor-pointer" title="展开">
        <ChevronRight className="w-5 h-5" />
      </button>
      <button onClick={() => { setCurrentConvId(null); nav('/chat'); }}
        className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 border-none cursor-pointer" title="新对话">
        <Plus className="w-5 h-5" />
      </button>
      {navItems.map(item => (
        <button key={item.path} onClick={() => nav(item.path)}
          className="p-2 rounded-lg hover:bg-gray-100 transition-colors border-none cursor-pointer" title={item.label}>
          <item.icon className="w-5 h-5" />
        </button>
      ))}
      <div className="flex-1" />
      <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-xs font-bold text-white mb-2">{initial}</div>
      <button onClick={() => { logout(); router.push('/login'); }}
        className="p-2 rounded-lg hover:bg-gray-100 transition-colors border-none cursor-pointer" title="退出">
    ...
    <>
      <div className="px-5 py-4 border-b shrink-0" style={{ borderColor: 'var(--c-border)' }}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold" style={{ letterSpacing: '-0.3px' }}>KnowFlow</h1>
            <p className="text-xs mt-0.5" style={{ color: 'var(--c-text-tertiary)' }}>知识库管理系统</p>
          </div>
          <button onClick={toggleSidebar} className="p-1 rounded hover:bg-gray-100 transition-colors max-md:hidden border-none cursor-pointer"
            style={{ color: 'var(--c-text-tertiary)' }} title="收起">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button onClick={closeMobile} className="p-1 rounded hover:bg-gray-100 transition-colors md:hidden border-none cursor-pointer"
        ...
        {navItems.map(item => {
          ...
          return (
            <button key={item.path} onClick={() => nav(item.path)}
              ...
              {active && <span className="absolute left-0 top-2 bottom-2 w-0.5 rounded-r-sm" style={{ background: 'var(--c-primary)' }} />}
              <item.icon className="w-[18px] h-[18px] shrink-0" style={{ opacity: active ? 1 : 0.7 }} />
      ...
      {isChat && (
        <div className="px-3 shrink-0">
          ...
          {kbs.length > 0 && (
            <select value={chatKbId} onChange={(e) => setChatKbId(e.target.value)}
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto px-3 pt-2 space-y-0.5">
            {filtered.length === 0 && search ? (
              <p className="text-xs text-center py-4" style={{ color: 'var(--c-text-tertiary)' }}>未找到匹配的对话</p>
            ) : filtered.map(conv => (
              <div key={conv.id} className="group relative rounded-md text-sm"
                style={{ background: currentConvId === conv.id ? 'var(--c-primary-subtle)' : 'transparent' }}>
                {renaming === conv.id ? (
                  <input autoFocus value={renameValue} onChange={e => setRenameValue(e.target.value)}
                    onBlur={() => submitRename(conv.id)} onKeyDown={e => e.key === 'Enter' && submitRename(conv.id)}
                    className="w-full rounded px-3 py-2 text-sm outline-none"
                    style={{ background: 'var(--c-surface)', border: '1px solid var(--c-primary)', color: 'var(--c-text)' }}
                ...
                <div className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:flex gap-0.5">
                  <button onClick={e => handleExport(conv.id, 'markdown', e)}
                    ...
                    style={{ color: 'var(--c-text-tertiary)', background: 'none' }} title="导出 MD">
      ...
      <div className="mt-auto px-5 py-4 border-t flex items-center gap-2.5 shrink-0"
