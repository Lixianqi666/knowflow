...
export default function DocumentsPage() {
  ...
  useEffect(() => { hydrate(); }, []);
  useEffect(() => {
    if (!useStore.getState().token) { router.replace('/login'); return; }
    api.get<KB[]>('/knowledge-bases')
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    for (const file of files) { try { await api.upload(file, currentKbId); } catch {} }
    setRefreshKey((k) => k + 1);
  }, [currentKbId]);
  ...
  return (
    <div className="flex h-screen" style={{ background: 'var(--c-bg)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="shrink-0 px-6 md:px-8 py-4 flex items-center justify-between gap-3 flex-wrap z-10"
          style={{
            background: 'rgba(255,255,255,.88)',
            backdropFilter: 'blur(8px)',
            borderBottom: '1px solid var(--c-border)',
          }}>
          <h2 className="text-xl md:text-2xl font-bold" style={{ letterSpacing: '-0.3px' }}>文档管理</h2>
          <div className="flex items-center gap-2.5">
            <div className="relative">
              ...
              <input value={searchVal} onChange={e => setSearchVal(e.target.value)}
                ...
                style={{
                  width: 220, padding: '8px 12px 8px 36px',
                  border: '1px solid var(--c-border)',
                  background: 'var(--c-bg)',
                  color: 'var(--c-text)',
                }}
                onFocus={e => { e.target.style.borderColor = 'var(--c-primary)'; e.target.style.boxShadow = '0 0 0 3px rgba(37,99,235,.12)'; e.target.style.background = 'var(--c-surface)'; }}
                onBlur={e => { e.target.style.borderColor = 'var(--c-border)'; e.target.style.boxShadow = 'none'; e.target.style.background = 'var(--c-bg)'; }} />
            </div>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="mb-4 rounded-xl p-4"
              ...
              <div className="flex items-center gap-2.5 flex-wrap">
                <select value={currentKbId} onChange={e => setCurrentKbId(e.target.value)}
                  ...
                  onFocus={e => { e.target.style.borderColor = 'var(--c-primary)'; e.target.style.boxShadow = '0 0 0 3px rgba(37,99,235,.12)'; }}
                ...
                <span className="ml-auto px-2.5 py-1 rounded-full text-xs border"
                  style={{
      ...
      <UploadDialog open={showUpload} kbId={currentKbId}
  ...
  return (
    <div className="grid grid-cols-4 gap-2">
      {[
        ...
        <div key={s.label} className="rounded-lg px-2.5 py-2" style={{ border: '1px solid var(--c-border)', background: '#fff' }}>
          <span className="block text-xs" style={{ color: 'var(--c-text-tertiary)', lineHeight: 1.2 }}>{s.label}</span>
          <span className="block mt-1 text-lg font-bold" style={{ color: s.color, lineHeight: 1 }}>{s.value}</span>
        </div>
      ))}
    </div>
  );
